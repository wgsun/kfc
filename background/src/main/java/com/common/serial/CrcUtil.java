...
public class CrcUtil {
  ...
  public static boolean checkCrc(byte[] data, int length){
    byte[] crc = intToBytes(calcCrc16(data,length));
    if(data[length-2]==crc[1] && data[length-1]==crc[0])
      return true;
    return false;
  }
  /**
  ...
  public static byte[]  getCrc(byte[] data){
    byte[] crc = intToBytes(calcCrc16(data));
   ...
   * @return
   */
  public static byte[]  getCrcTwo(byte[] data){
    byte[] crc = intToBytes(calcCrc16(data));
  ...
  /**
   * 获取返回的crc  byte数组    kfc传重仪
   * @param data
   * @return
   */
  public static byte[]  getCrcTrands(byte[] data){
    byte[] crc = intToBytes(calcCrc16(data));
    data[data.length-2]=crc[0];
    data[data.length-1]=crc[1];
    return data;
  ...
  public static int calcCrc16(byte[] data,int length) {
    return calcCrc16(data, 0, length-2);
  }
   ...
   * @return CRC16校验值
   */
  public static int calcResultCrc16(byte[] data) {
